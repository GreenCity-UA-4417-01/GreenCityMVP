...
public class HabitFactSpecification implements MySpecification<HabitFact> {
    ...
    @Override
    public Predicate toPredicate(Root<HabitFact> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
    ...
    private Predicate getTranslationPredicate(Root<HabitFact> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder, SearchCriteria searchCriteria) {
        Root<HabitFactTranslation> habitFactTranslationRoot = criteriaQuery.from(HabitFactTranslation.class);

        return searchCriteria.getValue().toString().trim().equals("")
    ...
    private Predicate getHabitIdPredicate(Root<HabitFact> root, CriteriaBuilder criteriaBuilder, SearchCriteria searchCriteria) {
        Join<HabitFact, Habit> habitJoin = root.join(HabitFact_.habit);

        return criteriaBuilder.equal(habitJoin.get(Habit_.id), searchCriteria.getValue());
    }
}
